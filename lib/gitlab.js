 ...
 */
/**
 * Returns the configured production tag suffix, defaulting to
 * -success-aws-prod-platform when unset or empty.
 */
function prodTagSuffix() {
  ...
  return rawSuffix || "-success-aws-prod-platform";
}

/**
 * Fetch all production platform tags, sorted by commit creation date descending
 * (newest first). Mirrors scripts/list-prod-tags.js. Pagination is capped at 10
 * pages (1000 tags) to stay within Vercel's serverless time limit.
 *
 * @param {string} projectId
 * @returns {Promise<Array<{name:string, date:Date}>>}
 */
export async function getProdTagsSorted(projectId) {
  const effectiveSuffix = prodTagSuffix();
  ...
  return allTags
    ...
    .map((t) => ({ name: t.name, date: new Date(t.commit?.created_at || t.created_at || 0) }))
...
export async function getPreviousReleaseTag(projectId, currentTag) {
  ...
  const prodTags = await getProdTagsSorted(projectId);
  ...
  const toDate = idx >= 0 ? prodTags[idx].date?.toISOString() ?? null : null;

  if (idx >= 0 && idx < prodTags.length - 1) {
    const prev = prodTags[idx + 1];
    return {
      name:     prev.name,
      fromDate: prev.date?.toISOString() ?? null,
      toDate,
    };
  }
