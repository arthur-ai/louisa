...
// ── Per-PR read helpers (used at merge time to generate the summaries log) ─────
...
export async function getPRComments(owner, repo, prNumber) {
  ...
  const [issueRes, reviewRes] = await Promise.all([
    fetch(`${GITHUB_API}/repos/${owner}/${repo}/issues/${prNumber}/comments?per_page=50`, { headers: h }),
    fetch(`${GITHUB_API}/repos/${owner}/${repo}/pulls/${prNumber}/comments?per_page=50`,  { headers: h }),
  ]);
...
export async function getPreviousReleaseTag(owner, repo, currentTag) {
  ...
  for (let page = 1; page <= 5; page++) {
    const url   = `${GITHUB_API}/repos/${owner}/${repo}/releases?per_page=100&page=${page}`;
  ...
  const currentDate = new Date(dateMap[currentTag]);
  const sorted = candidates
    .filter((r) => dateMap[r.tag_name])
    .sort((a, b) => new Date(dateMap[b.tag_name]) - new Date(dateMap[a.tag_name]));

  const previous = sorted.find((r) => new Date(dateMap[r.tag_name]) < currentDate);
  return previous ? previous.tag_name : null;
}

/**
 * Fetch the commit date for a tag (exported for use by the webhook).
 */
...
/**
 * List published releases sorted by tag commit date descending (newest first),
 * filtering out drafts, prereleases, and sdk-* tags. The release body (the
 * changelog Louisa previously generated) is included for each entry.
 *
 * @param {string} owner
 * @param {string} repo
 * @returns {Promise<Array<{tag_name:string, body:string, date:Date}>>}
 */
export async function listReleasesSorted(owner, repo) {
  const h        = await headers();
  const releases = [];
  for (let page = 1; page <= 5; page++) {
    const url   = `${GITHUB_API}/repos/${owner}/${repo}/releases?per_page=100&page=${page}`;
  ...
  const withDates = await Promise.all(
    candidates.map(async (r) => ({
      tag_name: r.tag_name,
      body:     r.body || "",
      date:     new Date((await getTagCommitDate(owner, repo, r.tag_name)) || 0),
    }))
  );

  return withDates.sort((a, b) => b.date - a.date);
}
...
export async function getPRsByDateRange(owner, repo, fromDate, toDate) {
  ...
  while (true) {
    ...
    if (res.status === 429) {
      ...
      const retry = await fetch(url, { headers: await headers() });
      if (!retry.ok) {
      ...
      for (const item of data.items || []) {
        if (!prs.has(item.number)) prs.set(item.number, formatSearchPR(item));
      }
