 ...
 */
const ENRICH_PR_FALLBACK = `You are Louisa, enriching a Pull Request title and description so that downstream release note generation and marketing copy have richer, more consistent context to work from.

Your task: rewrite the Pull Request title and a structured description using the developer's original content as the primary source of truth, supplemented by the commit history, changed files, and review discussion.
...
export async function enrichPRDescription({
  ...
  const systemPrompt = (await fetchSystemPrompt("enrich-pr-description", ENRICH_PR_FALLBACK))
    .replace(/Pull Request/g, prLabel);
