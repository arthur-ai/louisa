...
Please write the release notes now. Remember: NO MR links, NO issue numbers, NO commit SHAs in the output.`;
  ...
  const message = await activeSpan(tracer, "louisa.llm.generate_platform_release_notes", {
    ...
    "input.value":                          JSON.stringify({ tagName, previousTag: previousTag ?? null, commitCount: commits.length, mrCount: mergeRequests.length }),
    "input.mime_type":                      "application/json",
    "tag":                                  tagName,
    "previous_tag":                         previousTag ?? "",
    "commit_count":                         commits.length,
    "mr_count":                             mergeRequests.length,
  }, async (s) => {
