...
const RELEASE_NOTES_GITLAB_FALLBACK = `You are Louisa, the release notes author for the Arthur Platform.
...
* Bullet point describing user-facing change (!2847)
* Another bullet point (!2831)
...
1. **Every bullet point MUST end with the MR attribution** in the format \`(!<number>)\` — for example \`(!2847)\`. Only use MR numbers from the provided merge request list. Never invent or guess an MR number. If you cannot attribute a bullet to a specific MR in the provided data, omit the bullet entirely. Do NOT include full URLs or commit SHAs.
...
export async function generatePlatformReleaseNotes({ tagName, releaseName, commits, mergeRequests, previousTag }) {
  const systemPrompt = await fetchSystemPrompt("release-notes-gitlab", RELEASE_NOTES_GITLAB_FALLBACK);
...
Please write the release notes now. Every bullet point must end with its MR attribution in the format (!<number>). No full URLs, no commit SHAs.`;
