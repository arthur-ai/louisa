    ...
    blocks: [
      {
        ...
        text: {
          type: "plain_text",
          text: "🚀 New Arthur Engine Release",
          emoji: true,
        },
      },
      {
...
/**
 * Extract the theme line and section titles from the full notes
 * to create a concise Slack summary.
 */
function buildSlackSummary(notes) {
  const lines = notes.split("\n");
  const parts = [];

  // Find the theme line (first non-empty line after the date that isn't a heading)
  let foundDate = false;
  ...
  // Collect H2 section titles as bullet points
