...
// ── Slack Web API (interactive bot) ──────────────────────────────────────────────

/**
 * Post a message via the Slack Web API using the bot token.
 * Returns the message ts on success (for threading), or null on failure.
 *
 * @param {object} opts
 * @param {string} opts.channel    - channel id
 * @param {string} opts.text       - mrkdwn message text
 * @param {string} [opts.thread_ts] - parent ts to reply in-thread
 * @returns {Promise<string|null>}
 */
export async function postMessage({ channel, text, thread_ts }) {
  const token = process.env.SLACK_BOT_TOKEN;
  if (!token) {
    console.error("Louisa: SLACK_BOT_TOKEN not set, cannot post message");
    return null;
  }
  ...
  if (!data.ok) {
    console.error(`Louisa: chat.postMessage failed — ${data.error || res.status}`);
    return null;
  }
  return data.ts;
}
...
/**
 * Split a long markdown string into chunks under Slack's ~3000-char text limit,
 * breaking on paragraph boundaries (then lines, then hard-splitting if needed).
 *
 * @param {string} markdown
 * @param {number} [limit=2900]
 * @returns {string[]}
 */
export function chunkForSlack(markdown, limit = 2900) {
  const chunks = [];
  let current = "";
  ...
  for (const para of (markdown || "").split("\n\n")) {
    if (para.length > limit) {
      ...
      for (const line of para.split("\n")) {
        ...
        if (current.length + remaining.length + 1 > limit) flush();
        current += (current ? "\n" : "") + remaining;
      }
      continue;
    }
    if (current.length + para.length + 2 > limit) flush();
    current += (current ? "\n\n" : "") + para;
  }
  flush();
