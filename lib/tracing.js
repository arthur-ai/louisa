...
// Build an OTLP attribute object, preserving integer type for token counts etc.
function toAttrObj(key, value) {
...
export function createTrace() {
  ...
  function span(name, parentSpanId, attrPairs = []) {
    ...
    const s = {
      ...
      attributes: attrPairs.map(([k, v]) => toAttrObj(k, v)),
      status: { code: 1 },
    };
    ...
    return {
      spanId,
      addAttr(key, value) {
        s.attributes.push(toAttrObj(key, value));
        return this;
      },
      end(error) {
        if (ended) return;
